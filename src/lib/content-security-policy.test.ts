...
function expectNoUmamiOrigins(policy: string) {
  for (const directive of policy.split("; ")) {
    expect(directive.split(" ")).not.toContain("https://cloud.umami.is");
    expect(directive.split(" ")).not.toContain("https://gateway.umami.is");
  }
}

describe("content security policy", () => {
  test("allows the Umami script and ingestion origins in production", () => {
    ...
    expect(directiveTokens(policy, "connect-src")).toEqual(["'self'", "https://gateway.umami.is"]);
  });
