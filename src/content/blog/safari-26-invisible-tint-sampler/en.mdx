...
<Figure
  ...
  caption="21:21 on July 25: the final Simulator pass in both themes, with matching colors on the page and Safari chrome."
...
The work from the first recorded investigation to the invisible sampler
stretched across weeks.

## Safari sampled the viewport edge
...
WebKit engineer Wenson Hsieh explains the intended behavior: Safari extends a
solid color near an obscured viewport edge when a `fixed` or `sticky` element is
attached there.
...
Safari determined the tint from the page surface at the viewport edge.

## The fixes that almost worked

Each failed version helped me rule out one wrong assumption.
...
Safari in the macOS Simulator loaded `localhost:3000`, and Safari Web Inspector
attached to that exact page. I could edit the CSS, refresh, inspect computed
styles, switch the theme, open and close the drawer, and rotate the simulated
phone without deploying every CSS guess to Vercel or leaving the local
development loop.
...
I used the Simulator to filter out bad ideas, then pushed a preview once a
version survived the full local matrix. I still used a real iPhone for the final
check. This was much faster than deploying, testing, adjusting, and redeploying
after every CSS guess.

## What WebKit checks at the edge

I found the answer in `LocalFrameView::fixedContainerEdges`. I traced the current
...
visual spacing choice. It is the smallest integer CSS height above the cutoff.

WebKit gets the color and paints the background through separate code paths. The
edge sampler reads the renderer's computed `background-color`. Normal page
painting applies background clipping later.
...
handles text clipping separately from the fixed-edge eligibility code. I use
that separation in the sampler.

The empty text clip leaves no visible pixels. The huge `z-index` keeps the empty
sampler as the frontmost edge hit even while a drawer or dialog is open.
...
I still call this a hack because it relies on a private browser heuristic and
separate paths for style inspection and painting. Users cannot see the sampler,
and Safari now keeps the tint consistent through every state that broke the
earlier versions. After weeks of iteration and almost five hours in the final
session, I am satisfied with that result.
